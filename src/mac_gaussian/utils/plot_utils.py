...
def load_volume(path):
    """Load volume from .npy or image stack directory."""
    if path.endswith(".npy"):
        volume = np.load(path)
        # volume = np.rot90(volume, k=1, axes=(0,2))
        return volume
    else:
        # assume directory containing image slices
        import os
        files = sorted(os.listdir(path))
        imgs = [np.array(Image.open(os.path.join(path, f))) for f in files]
        return np.stack(imgs, axis=2)  # H × W × 

def show_three_volumes(
    path1,
    path2,
    path3,
    cmap="viridis",
    ...
    # Load volumes
    vol1 = load_volume(path1)
    vol2 = load_volume(path2)
    vol3 = load_volume(path3)

    # Axis transform (same logic as original)
    if axis == 0:
        vol1 = vol1.transpose([1, 2, 0])
        vol2 = vol2.transpose([1, 2, 0])
        vol3 = vol3.transpose([1, 2, 0])
    elif axis == 1:
        vol1 = vol1.transpose([0, 2, 1])
        vol2 = vol2.transpose([0, 2, 1])
        vol3 = vol3.transpose([0, 2, 1])

    # Plot 3 volumes side-by-side
    fig, ax = plt.subplots(1, 3)
    plt.subplots_adjust(bottom=0.25)

    initial_slice = vol1.shape[2] // 2

    img1 = ax[0].imshow(vol1[:, :, initial_slice], cmap=cmap,
                        vmin=0.0, vmax=vol1.max() if vmax is None else vmax)
    ax[0].set_title(title1)

    img2 = ax[1].imshow(vol2[:, :, initial_slice], cmap=cmap,
                        vmin=0.0, vmax=vol2.max() if vmax is None else vmax)
    ax[1].set_title(title2)

    img3 = ax[2].imshow(vol3[:, :, initial_slice], cmap=cmap,
                        vmin=0.0, vmax=vol3.max() if vmax is None else vmax)
    ax[2].set_title(title3)

    # Colorbars
    fig.colorbar(img1, ax=ax[0], orientation="horizontal")
    fig.colorbar(img2, ax=ax[1], orientation="horizontal")
    fig.colorbar(img3, ax=ax[2], orientation="horizontal")

    # Slider
    ax_slider = plt.axes([0.1, 0.01, 0.65, 0.03])
    max_slice = max(vol1.shape[2], vol2.shape[2], vol3.shape[2]) - 1
    slice_slider = Slider(ax_slider, "Slice", 0, max_slice,
    ...
    def update(val):
        ...
        if s < vol1.shape[2]:
            img1.set_array(vol1[:, :, s])
        if s < vol2.shape[2]:
            img2.set_array(vol2[:, :, s])
        if s < vol3.shape[2]:
            img3.set_array(vol3[:, :, s])
        fig.canvas.draw_idle()
