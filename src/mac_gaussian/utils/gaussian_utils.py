...
def inverse_softmax(y, alpha = 1.0, normalize = "min0"):
    """
    y: 1D probability vector (sum=1, all >0)
    alpha: temperature scale
    normalize:
        "min0"  -> shift so that min(x)=0
        "mean0" -> shift so that mean(x)=0
    """
