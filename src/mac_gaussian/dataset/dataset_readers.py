...
sys.path.append("./")
from mac_gaussian.utils.graphics_utils import BasicPointCloud, fetchPly
...
def readBlenderInfo(path, eval):
    ...
    if osp.isfile(path_vol_mask):
        ...
        vol_mask = None     # torch.zeros_like(vol_gt).float().cuda()

    path_vol_fdk = osp.join(osp.dirname(meta_data["vol"]), "vol_fdk.npy")
    if osp.isfile(path_vol_fdk):
        vol_fdk = torch.from_numpy(np.load(path_vol_fdk)).float().cuda()
    else:
        print(f"Warning: {path_vol_fdk} not found, using vol_gt as vol_fdk")
        vol_fdk = vol_gt
