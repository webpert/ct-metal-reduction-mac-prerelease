	...
	)
{
	...
	__shared__ float collected_opacity_res[BLOCK_SIZE * BHC_MAC_BASIS_COUNT];	// cvpr
	__shared__ float collected_mu[BLOCK_SIZE];

	// Initialize helper variables
	uint32_t contributor = 0;
	uint32_t last_contributor = 0;
	float sum_opacity[BHC_ETA_COUNT];

	for (int k=0; k<BHC_ETA_COUNT; k++)	// cvpr
	{
		sum_opacity[k] = 0.0f;
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float u = collected_opacity_res[j];
			...
			//! Simply add all alphas for X-ray imaging. TODO: computation can be optimized.
			for (int k=0; k<BHC_ETA_COUNT; k++)		// cvpr
			{
				...
				sum_opacity[k] += (alpha * h);
			}
			// int base_opacity_res_idx = j * BHC_MAC_BASIS_COUNT;

			// for (int m=0; m<BHC_MAC_BASIS_COUNT; m++) {
			// 	float coeff = alpha * collected_opacity_res[base_opacity_res_idx + m];
			// 	int base_mac_idx = m * BHC_ETA_COUNT;
			// 	for (int k=0; k<BHC_ETA_COUNT; k++) {
			// 		sum_opacity[k] += mac_basis[base_mac_idx + k] * coeff;
			// 	}
			// }
	...
	if (inside)
	{
		...
		for (int k=0; k<BHC_ETA_COUNT; k++)
		{
			exponent = -sum_opacity[k];
			B_k = bhc_eta[k] * exp(exponent);
			A += B_k;
			out_B_k[k * H * W + pix_id] = B_k;
			// out_C_k[k * H * W + pix_id] = B_k;	// debug port
			// debug = fminf(debug, exponent);
		}
		A = fmaxf(A, eps);		// to avoid exception
		out_color[pix_id] = -log(A);
		out_A[pix_id] = A;
		// out_D[pix_id] = A;		// debug port
	}
}
