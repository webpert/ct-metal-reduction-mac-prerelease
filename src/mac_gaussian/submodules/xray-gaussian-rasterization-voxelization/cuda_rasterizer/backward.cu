...
renderCUDA(
	...
	const float* __restrict__ mac_basis,		// cvpr, [BHC_MAC_BASIS_COUNT, BHC_ETA_COUNT]
	const float2* __restrict__ points_xy_image,
	const float4* __restrict__ conic_opacity,
	const float* __restrict__ opacity_res,	// cvpr, [G, M]
...
{	
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float u = collected_opacity_res[j];
			const float s = collected_mu[j];
			const float delta = con_o.w;

			// cvpr, TODO, should be checked if necessary,
			// if this is activated, unnecessary gaussians are skipped.
			// const float alpha = b * s * G;
			// if (alpha <0.00001f)
			// 	continue;
			...
			const float sG = s * G;
			const float dG = delta * G;
			const float sd = s * delta;
			const float sdG = sd * G;
			float C = 0.0f;
			float inverse_A;
			if (abs(A_pixel) >= 1e-35f)
				inverse_A = (1.0f)/(A_pixel);
			else
				inverse_A = 1e+35f;
			...
			const float coeff_common = inverse_A * C;
			const float dL_dd = dL_dalpha * coeff_common * sG;
			const float dL_ds = dL_dalpha * coeff_common * dG;
			const float dL_dG = dL_dalpha * coeff_common * sd;
			float B_k_dh_du = 0.0f;
			for (int k=0; k<BHC_ETA_COUNT; k++)
			{
				float b_bone = mac_basis[BHC_ETA_COUNT + k];
				float b_water = mac_basis[k];
				float b_fe = mac_basis[2*BHC_ETA_COUNT + k];
				float dh_du = 2.0f * ((1.0f - u) * (b_bone - b_water) + (u * (b_fe - b_bone)));
				B_k_dh_du += (B_pixel[k] * dh_du);
			}
			float dL_du = dL_dalpha * (sdG * B_k_dh_du * inverse_A);
			...
			atomicAdd(&(dL_dopacity[global_id]), dL_dd);	// dL_da
			atomicAdd(&(dL_dopacity_res[global_id]), dL_du);
			atomicAdd(&(dL_dmu[global_id]), dL_ds);	// dL_ds			

			// for (int k=0; k<BHC_ETA_COUNT; k++)
			// {
			// 	dL_dbhc_eta_pixel[k*H*W + pix_id] = dL_dalpha * (-1.0) * inverse_A * B_pixel[k] / bhc_eta[k];
			// }
