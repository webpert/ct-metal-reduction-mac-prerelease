...
renderCUDA(
	...
	const float* __restrict__ opacities_res,
	...
	float* __restrict__ dL_dopacities_res)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK3D_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK3D_SIZE, toDo); j++)
		{
			...
			// opacities_res[global_id * BHC_MAC_BASIS_COUNT + m];	
			float u = opacities_res[global_id];
			float dL_dG = 0.0;
			float dL_do = 0.0;
			for (int k = 0; k < C; k++)
			{
				...
				dL_dG += (opa * h * dL_dpixel[k]);
				dL_do += (G * h * dL_dpixel[k]);
			}
			...
			{
				local_sum += (opa * G * dL_dpixel[ch]);
			}			
			atomicAdd(&dL_dopacities_res[global_id], local_sum);
...
void BACKWARD::render(
	...
	float* dL_dconic3D,
	float* dL_dopacity,
	float* dL_dopacities_res
	)
{
	renderCUDA<BHC_ETA_COUNT> << <grid, block >> >(
		ranges,
		point_list,
		nVoxel_x, nVoxel_y, nVoxel_z,
		sVoxel_x, sVoxel_y, sVoxel_z,
